...
Launch a Task subagent with `subagent_type: generalPurpose`. Include the full persona from `agents/web-performance-auditor.md` (from the agent-skills checkout or project copy). Pass it explicitly:
...
If the Task tool is unavailable, adopt the `web-performance-auditor` persona directly in this session and run the audit.
