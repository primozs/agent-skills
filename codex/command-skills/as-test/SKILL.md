---
name: as-test
description: >-
  Runs the TDD workflow — write failing tests, implement, verify. Use when the user
  invokes $as-test or asks to test-drive a change or fix a bug with Prove-It.
---

# as-test

Codex entry point for the agent-skills lifecycle. Invoke as `$as-test`.

## Prerequisite

The **agent-skills** Codex plugin must be installed and enabled.

## User input

Text after `$as-test` is **$ARGUMENTS** — the feature, behavior, or bug to test-drive.

## Workflow

1. Invoke and fully follow the `test-driven-development` skill from the agent-skills plugin (`$test-driven-development`).
2. Apply the kickoff steps below.

For new features:
1. Write tests that describe the expected behavior (they should FAIL)
2. Implement the code to make them pass
3. Refactor while keeping tests green

For bug fixes (Prove-It pattern):
1. Write a test that reproduces the bug (must FAIL)
2. Confirm the test fails
3. Implement the fix
4. Confirm the test passes
5. Run the full test suite for regressions

For browser-related issues, also invoke `$browser-testing-with-devtools` to verify with Chrome DevTools MCP when available.
