...
Codex entry point for the agent-skills lifecycle. Invoke as `$as-build` or `$as-build auto`.
...
1. Invoke and fully follow `incremental-implementation` and `test-driven-development` from the agent-skills plugin (`$incremental-implementation`, `$test-driven-development`).
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix → invoke `$debugging-and-error-recovery`
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → invoke `$doubt-driven-development` and get explicit sign-off before continuing
...
If any step fails, invoke `$debugging-and-error-recovery`.
