...
Codex entry point. Invoke as `$as-ponytail-audit`.
...
The **agent-skills** Codex plugin must be installed and enabled. Optionally install
`ponytail-audit` under `~/.agents/skills/` or `~/.codex/skills/` for the full audit skill.
...
2. If `ponytail-audit` is installed, invoke and follow it
   (`$ponytail-audit`, or `~/.agents/skills/ponytail-audit/SKILL.md` / `~/.codex/skills/ponytail-audit/SKILL.md`),
...
Prefer the shortest correct cut (ponytail ladder). Scope: over-engineering only — do not expand into unrelated refactors.
